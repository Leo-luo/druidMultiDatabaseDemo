...
@Component 
public class TestDaoImplWrite {
	 private static Logger logger = Logger.getLogger(TestDaoImplRead.class);
	 
	 private JdbcTemplate jdbcTemplateWrite;

     @Autowired
     public void setJdbcTemplateWrite(@Qualifier("dataSourceWrite") DataSource dataSource) {
        this.jdbcTemplateWrite = new JdbcTemplate(dataSource);
     }
	 ...
	 /**
		...
		public List<AdminListDto> adminList(AdminDataTablesReq req){
			try{
				...
				logger.info("TestDaoImplWrite.adminList sql = " + sql.toString());
			    return jdbcTemplateWrite.query(sql.toString(), new BeanPropertyRowMapper(AdminListDto.class));
			}catch(Exception e){
				e.printStackTrace();
				return null;
			}
		...
		public int adminCount(AdminDataTablesReq req){
			try{
				...
				logger.info("TestDaoImplWrite.adminCount sql = " + sql.toString());
			    return jdbcTemplateWrite.queryForInt(sql.toString());
			}catch(Exception e){
				e.printStackTrace();
				return 0;
			}
